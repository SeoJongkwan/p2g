...
def create_model(rid):
    ...
    scaler = MinMaxScaler()
    train_scaled = scaler.fit_transform(train.astype(float))
    test_scaled = scaler.fit_transform(test.astype(float))
    print(test_scaled)
...
# best_model = aml.leader.model_id
# print("Best Model: ", best_model)
# save_model = h2o.download_model(best_model, model_path + r)
